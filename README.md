...
- 不在 argparse 中提供 `--db-config-json`，以免在 `import utils.*` 之后才改环境变量导致配置已固化；请用 **`LIFT_DB_CONFIG_JSON`** 或默认路径。
...
4. 配置 `feature_engineering/db_config.json` 或设置 `LIFT_DB_CONFIG_JSON`。
